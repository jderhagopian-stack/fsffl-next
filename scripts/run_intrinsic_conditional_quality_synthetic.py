...
def conditional_c2(mu: float, sd: float, state: str) -> tuple[float, float]:
    if state == "out":
        return 0.0, 1.0
    lo, hi = STATES[state]
    ...
    return (TEAM_COUNT * em + 0.5 * max(0.0, ex)) / (TEAM_COUNT + 1.0), mass


def future_value(mu: float, sd: float, probs: dict[str, float]) -> float:
    return sum(probs.get(state, 0.0) * conditional_c2(mu, sd, state)[0] for state in STATES)


def three_year(y1: float, y2: tuple[float, float], y3: tuple[float, float], p2: dict[str, float], p3: dict[str, float]) -> float:
    return c2_at_x(y1) + DISCOUNT * future_value(*y2, p2) + DISCOUNT**2 * future_value(*y3, p3)


def main() -> None:
    same_probs = {"usable": 0.15, "starter": 0.60, "premium": 0.20, "elite": 0.05}
    high_quality = three_year(165, (205, 35), (210, 35), same_probs, same_probs)
    low_quality = three_year(165, (150, 35), (145, 35), same_probs, same_probs)
    ...
    stable_qb = three_year(225, (220, 30), (215, 30), stable_probs, stable_probs)
    high_variance_backup = three_year(90, (150, 110), (155, 115), backup_probs, backup_probs)
    ...
    credible_dev_qb = three_year(70, (175, 65), (195, 65), dev_qb_probs2, dev_qb_probs3)
    ...
    strong_dev_wr = three_year(55, (165, 60), (185, 60), strong_wr_probs, strong_wr_probs)
    weak_dev_wr = three_year(55, (105, 60), (110, 60), weak_wr_probs, weak_wr_probs)

    fixed_probs = {"usable": 0.20, "starter": 0.60, "premium": 0.18, "elite": 0.02}
    low_variance = three_year(150, (160, 25), (160, 25), fixed_probs, fixed_probs)
    high_variance = three_year(150, (160, 90), (160, 90), fixed_probs, fixed_probs)
    ...
    result = {
        "same_state_different_quality": {
            ...
            "pass": high_quality >= 1.10 * low_quality,
        },
        "existing_gates": {
            ...
            "generic_variance_uplift_pct": (high_variance / low_variance - 1.0) * 100.0,
            "generic_variance_no_free_option_pass": high_variance <= 1.05 * low_variance,
        },
        "notes": {
            "state_probabilities_are_external_and_fixed": True,
            "forecast_distribution_only_sets_quality_inside_state": True,
            "market_inputs_used": False,
        },
    }
    out = Path("artifacts/conditional-d2-synthetic/conditional_d2_synthetic.json")
