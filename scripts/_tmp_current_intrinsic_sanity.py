...
ROWS=[
('Josh Allen','4984','QB',30,388.0555,208.8451987163231,7,1.0),('Lamar Jackson','4881','QB',29,329.128,177.13137054649653,32,.933333333333333),('Drake Maye','11564','QB',24,334.793,180.18018199415795,3,.983333333333333),('Dak Prescott','3294','QB',33,312.358,168.1060275672764,135,.9),('Sam Darnold','4943','QB',29,287.6005,154.78194117442965,3,.683333333333333),('Drew Lock','5854','QB',29,2.7755,1.4937292450104553,42,0.0),
('Bijan Robinson','9509','RB',24,331.248,188.09693698845163,8,None),('Quinshon Judkins','12512','RB',22,210.4705,119.51425027902931,36,None),('Rhamondre Stevenson','7611','RB',28,165.103,93.75262216709031,120,None),('Tyler Allgeier','8132','RB',26,89.6405,50.90175179959818,151,None),('Jonathon Brooks','11583','RB',23,165.548,94.00531240811775,46,None),('Derrick Henry','3198','RB',32,259.1655,147.1653767662916,45,None),('Trevor Etienne','12531','RB',22,7.5605,4.293178802894473,None,None),
('CeeDee Lamb','6786','WR',27,214.4855,110.92139549549066,17,None),('Tee Higgins','6801','WR',27,183.628,94.9634078389726,33,None),('Zay Flowers','9997','WR',26,197.848,102.317295369579,22,None),('DeVonta Smith','7525','WR',27,185.5505,95.95763067846562,10,None),('KC Concepcion','13298','WR',21,123.068,63.64474195616507,24,None),('Jaxon Smith-Njigba','9488','WR',24,246.3755,127.41334158206152,20,None),('Troy Franklin','11627','WR',23,42.433,21.94426930986083,102,None),('Devin Duvernay','6847','WR',29,1.4605,1.02435345462394,92,None),
('Brock Bowers','11604','TE',23,189.228,106.98253129903512,13,None),('Kyle Pitts','7553','TE',25,146.353,82.74258779465877,4,None),('Dallas Goedert','5022','TE',31,125.6005,71.0098897754268,49,None),('Elijah Arroyo','12521','TE',23,55.1805,31.197019301300063,50,None),("Ja'Tavion Sanders",'11600','TE',23,13.1855,7.454595337071828,101,None)]
def build(row):
 ...
 return path,estimate_intrinsic_value_v2(player_path=path,player_state=state)
print('| Player | Pos | Y1 | Y2 | Y3 | post-Y3 | factor | residual raw | raw Intrinsic | display |')
print('|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|')
for row in ROWS:
 path,e=build(row);m=[p.distribution.mean for p in path.horizons];print(f'| {row[0]} | {row[2]} | {m[0]:.1f} | {m[1]:.1f} | {m[2]:.1f} | {e.raw_terminal_value:.1f} | {e.terminal.applied_factor:.3f} | {e.residual_fundamental_value:+.1f} | {e.fundamental_value:.1f} | {e.display_value} |')
