...
def main():
    ...
    result={"study":"FSFFL first-party FUMBLES_LOST model","target":{"coordinate":"FUMBLES_LOST","cutoff_week":CUTOFF,"formula":"17 * exact lost fumbles in Weeks 3+ / 15","semantics":"exact lost fumbles; total fumbles forbidden"},"folds":[{"held_out":y,"training_seasons":[x for x in SEASONS if x<y]} for y in OOT],"features":{"QB":"attempts+sacks_suffered+carries","RB_WR_TE":"carries+receptions","current":"Weeks 1-2 only","history":"strictly prior seasons only","shrink_opportunities":SHRINK_OPPS,"role_prior_games":ROLE_PRIOR_GAMES,"calibration":"one global multiplier learned only from prior pseudo-current seasons"},"calibration_scales":{str(y):calibration_scale(rows_by,y) for y in OOT+(2026,)},"gate":gg,"cold_start_stress":stress}
