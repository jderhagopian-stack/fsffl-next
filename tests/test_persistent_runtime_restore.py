...
def _lifecycle_record(*, status: str, fingerprint: str) -> ReusableArtifactRecord:
...
def test_terminal_failed_refresh_restores_same_league_last_good() -> None:
    persistence = MemoryPersistence()
    last_good = _league_state()
    persist_runtime_snapshot(
        persistence,
    ...
    persistence.put_artifact(
        ReusableArtifactRecord(
            ...
            computed_at=datetime(2026, 9, 8, 12, 1, tzinfo=UTC),
        )
    )
    partial = last_good.model_copy(
        update={"as_of": datetime(2026, 9, 8, 12, 5, tzinfo=UTC)}
    )
    persist_runtime_snapshot(
        persistence,
        user_id="jimmy",
        league_state=partial,
        selected_team_id="t1",
    )
    persistence.put_artifact(
        _lifecycle_record(status="failed", fingerprint="job-failed")
    )
...
def test_terminal_failed_refresh_never_rolls_back_across_leagues() -> None:
    persistence = MemoryPersistence()
    old_league = _league_state()
    persist_runtime_snapshot(
        persistence,
        user_id="jimmy",
        league_state=old_league,
        selected_team_id="t2",
    )
    persistence.put_artifact(
        ReusableArtifactRecord(
            key=ArtifactKey(
                ...
                scope_kind=LAST_GOOD_SCOPE_KIND,
                scope_id="jimmy",
                input_fingerprint=old_league.state_id,
                model_version=LAST_GOOD_MODEL_VERSION,
            ),
            payload={
                "league_state": old_league.model_dump(mode="json"),
                "selected_team_id": "t2",
            },
            computed_at=datetime(2026, 9, 8, 12, 1, tzinfo=UTC),
        )
    )
    new_league = old_league.model_copy(
        update={
            "league": old_league.league.model_copy(
                update={
    ...
    persist_runtime_snapshot(
        persistence,
        user_id="jimmy",
        league_state=new_league,
        selected_team_id="n1",
    )
    persistence.put_artifact(
        _lifecycle_record(status="failed", fingerprint="job-new-league-failed")
    )
    ...
    assert restored.league_state.state_id == new_league.state_id
    assert restored.selected_team_id == "n1"
