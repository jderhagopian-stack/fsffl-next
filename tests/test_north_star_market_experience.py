...
def test_market_release_is_eager_and_single_generation() -> None:
    ...
    assert 'north_star_market.css?v=20260925-last-good-recovery1' in source
    assert 'north_star_market.js?v=20260925-last-good-recovery1' in source
    versions = {token.split("?v=")[1].split('"')[0] for token in source.split() if "?v=" in token}
    assert versions == {"20260925-last-good-recovery1"}
