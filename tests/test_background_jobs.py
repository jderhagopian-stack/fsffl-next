...
def test_job_failure_is_persisted_for_polling_client() -> None:
    ...
    assert current.failure_stage == IntelligenceJobPhase.BUILDING_FORECASTS
...
def test_failure_preserves_exact_runtime_stage_across_restart() -> None:
    persistence = _LifecyclePersistence()
    coordinator = IntelligenceJobCoordinator(
        max_workers=1,
    ...
    coordinator.start(user_id="u-stage", league_state_id="state-1", work=work)
    ...
    assert failed.failure_stage == IntelligenceJobPhase.RUNNING_SIMULATION

    restarted = IntelligenceJobCoordinator(
        max_workers=1,
        persistence_store=persistence,  # type: ignore[arg-type]
    )
    restored = restarted.current("u-stage")
    assert restored is not None
    assert restored.status == IntelligenceJobStatus.FAILED
    assert restored.failure_stage == IntelligenceJobPhase.RUNNING_SIMULATION
