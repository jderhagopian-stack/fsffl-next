...
- **Hodor:** PR #242 is merged at `6bc184487e7e6619350337b374f67976457f8317` and live as `dep-darbvcm0tbcc73b02bg0`. Production restores `sleeper:1397623301961981952`, State `3da88ba8...`, selected franchise `jder52`, and a 16-player roster while truthfully identifying Forecast as the blocked stage. Hodor's remaining full-intelligence blocker is Forecast/Product authority, not unfinished Performance implementation.
- **Forecast partial-coverage rule:** valid governed coordinates must remain usable even when some league scoring events are unsupported or inherently unforecastable. Missing coordinates must be explicit and never silently zeroed, but bounded rare-event gaps must not erase otherwise valid shared player/stat Forecasts. Material gaps may block only the affected subject/consumer; partial/provisional outputs must carry machine-readable coverage and downstream authority limits.
...
8. Resume new-league 7/7 lifecycle acceptance only after current-forward K/DST Forecast authority is actually promoted.
