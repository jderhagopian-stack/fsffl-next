...
function homeRefreshReadinessStrip(){
  const node=document.querySelector('#home-attention .home-readiness-strip');if(!node)return;
  const status=homeReadinessSnapshot(),pct=status.total?Math.max(0,Math.min(100,(status.step/status.total)*100)):0;
...
function homeLoadingMarkup(){
  return '<section class="home-north-star"><div class="home-loading"><i></i><strong>Building your command center</strong><span>Reading current governed team intelligence.</span></div>'+homeReadinessMarkup()+'</section>';
}
...
@media(min-width:761px){.home-attention{padding-top:14px}.home-north-star{grid-template-columns:1fr 1fr}.home-identity,.home-pressure,.home-outlook,.home-roster-shape,.home-secondary,.home-around,.home-readiness-strip{grid-column:1/-1}.home-outlook-grid{max-width:520px}.home-position-grid{max-width:560px}}
