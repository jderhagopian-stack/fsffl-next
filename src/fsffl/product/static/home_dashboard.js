...
function homeLoadingMarkup(){
  return '<section class="home-north-star"><div class="home-loading"><i></i><strong>Building your command center</strong><span>Reading already-attached governed State, Team Utility and Simulation evidence.</span></div></section>';
}
...
function renderFsfflHomeNorthStar(){
  ...
  container.innerHTML=`<section class="home-north-star">
    ...
    <details class="home-evidence"><summary>Evidence & readiness</summary><p>Home summarizes canonical State, existing position-strength and Team Utility evidence, and the matching current Simulation when attached. It launches no Opportunity Search, bilateral Decision evaluation, Value build, Forecast refresh, or changed-state Simulation.</p><p>State: ${homeEscape(String(payload.league_state_id||'unavailable').slice(0,12))}… · Simulation: ${homeEscape(payload.simulation?.status||'unavailable')}.</p></details>
  </section>`;
...
@media(min-width:761px){.home-attention{padding-top:14px}.home-north-star{grid-template-columns:1fr 1fr}.home-identity,.home-pressure,.home-outlook,.home-roster-shape,.home-secondary,.home-around,.home-evidence{grid-column:1/-1}.home-outlook-grid{max-width:520px}.home-position-grid{max-width:560px}}
