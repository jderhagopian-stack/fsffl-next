...
let accessibilityScriptPromise=null;
...
function ensureAccessibilityScript(){return lazyProductScript('installFsfflAccessibility','/static/accessibility.js','Unable to load accessibility presentation module',()=>accessibilityScriptPromise,value=>accessibilityScriptPromise=value)}
...
window.addEventListener('load',()=>{rebuildProductNavigation();ensureHomeScript().then(()=>window.installFsfflHomeExperience?.()).catch(()=>{});ensureAccessibilityScript().then(()=>window.installFsfflAccessibility?.()).catch(()=>{})});window.addEventListener('fsffl:product-context-updated',()=>{rebuildProductNavigation();window.installFsfflAccessibility?.()});setTimeout(rebuildProductNavigation,0);
