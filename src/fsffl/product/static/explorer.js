...
function renderPlayersAssetsExplorer(values){
  ...
  panel.innerHTML=`<div class="panel-header"><div><p class="eyebrow">Players & Assets</p><h2>The league market, in one place</h2></div><span class="status-chip">Authoritative Value</span></div><p class="lead">Find any rostered player or owned pick without digging through team pages. Age and ownership come from canonical State; FSFFL Value and Market Percentile remain separate governed measures.</p><div class="explorer-controls" style="display:grid;grid-template-columns:minmax(180px,2fr) repeat(3,minmax(120px,1fr));gap:8px;margin:16px 0"><input id="explorer-search" type="search" placeholder="Search player, pick or team" aria-label="Search assets"><select id="explorer-type"><option value="">All assets</option><option value="player">Players</option><option value="pick">Picks</option></select><select id="explorer-position"><option value="">All positions</option>${positions.map(value=>`<option>${explorerEscape(value)}</option>`).join('')}</select><select id="explorer-owner"><option value="">All teams</option>${owners.map(([id,name])=>`<option value="${explorerEscape(id)}">${explorerEscape(name)}</option>`).join('')}</select></div><div style="display:flex;justify-content:space-between;align-items:center;gap:10px;margin-bottom:8px"><small id="explorer-result-count"></small><small>Tap column labels to sort</small></div><div class="table-wrap"><table><thead><tr><th>${explorerSortButton('Asset','name',fsfflExplorerState.assetSort)}</th><th>${explorerSortButton('Type / Pos','position',fsfflExplorerState.assetSort)}</th><th>${explorerSortButton('Age','age',fsfflExplorerState.assetSort)}</th><th>${explorerSortButton('Team','owner',fsfflExplorerState.assetSort)}</th><th>${explorerSortButton('Roster slot','slot',fsfflExplorerState.assetSort)}</th><th>${explorerSortButton('FSFFL Value','value',fsfflExplorerState.assetSort)}</th><th>${explorerSortButton('Market percentile','market_percentile',fsfflExplorerState.assetSort)}</th></tr></thead><tbody id="explorer-asset-body"></tbody></table></div><details style="margin-top:16px"><summary style="cursor:pointer">Evidence & data sources</summary><div style="padding-top:10px"><p><strong>Value sources loaded:</strong> ${explorerEscape(successful)}</p><p><strong>Unavailable/partial sources:</strong> ${explorerEscape(failed)}</p><p><strong>Age / ownership / roster slot:</strong> canonical point-in-time State. <strong>FSFFL Value:</strong> governed NEXT-3 market-cardinal output supplied by the backend. <strong>Market percentile:</strong> separate multi-source market-position evidence.</p><p><strong>Value model:</strong> ${explorerEscape(values.model_version||'—')} · <strong>Cardinal coverage:</strong> ${values.cardinal_player_coverage==null?'—':explorerPercent(values.cardinal_player_coverage,1)}</p></div></details>`;
