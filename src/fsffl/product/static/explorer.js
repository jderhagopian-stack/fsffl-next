...
function explorerPlayerProjection(player){const observations=player?.forecasts||[];const regularSeason=observations.find(item=>item.metric==='fantasy_points'&&item.horizon==='fantasy_regular_season');const season=observations.find(item=>item.metric==='fantasy_points'&&item.horizon==='season');const anyFantasy=regularSeason||season||observations.find(item=>item.metric==='fantasy_points');return typeof anyFantasy?.distribution?.mean==='number'?anyFantasy.distribution.mean:null}
