const fsfflExplorerState={assetRows:[],teamRows:[],assetSort:{key:'value',direction:'desc'},teamSort:{key:'expected_wins',direction:'desc'},teamViewSource:''};
...
function explorerPlayerProjection(player){const observations=player?.forecasts||[];const season=observations.find(item=>item.metric==='fantasy_points'&&item.horizon==='season');const anyFantasy=season||observations.find(item=>item.metric==='fantasy_points');return typeof anyFantasy?.distribution?.mean==='number'?anyFantasy.distribution.mean:null}
...
function filteredAssetRows(){
  ...
  const role=document.querySelector('#explorer-role')?.value||'';
  return explorerSorted(fsfflExplorerState.assetRows.filter(row=>{
    ...
    if(role==='starter'&&!row.projected_starter)return false;
    if(role==='reserve'&&(row.kind!=='player'||row.projected_starter))return false;
    return true;
