...
async function analyzeTradeDraft(){
  const button=qs('#analyze-trade');if(button){button.disabled=true;button.textContent='Analyzing…'}renderTradeAnalysisNotice('Submitting the draft to authoritative Trade Decision…');
  try{const result=await api('/api/trade-center/analyze',{method:'POST',body:JSON.stringify({counterparty_team_id:tradeUiState.counterpartyTeamId,focal_asset_refs:[...tradeUiState.focalSelected],counterparty_asset_refs:[...tradeUiState.counterpartySelected]})});const panel=qs('#trade-analysis-empty');if(panel)panel.innerHTML=`<pre style="white-space:pre-wrap;text-align:left;width:100%;margin:0">${escapeHtml(JSON.stringify(result,null,2))}</pre>`}
  catch(error){renderTradeAnalysisNotice(`Trade Decision is not available for this draft yet: ${error.message}. Your visual draft is preserved.`)}finally{if(button){button.textContent='Analyze Trade';updateAnalyzeTradeState()}}
}
