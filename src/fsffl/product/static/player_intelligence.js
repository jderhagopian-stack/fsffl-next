...
(function(){
  ...
  function historyTable(){
    ...
    return `<div class="pi-history-list">${rows.map(row=>`<details class="pi-history-row" data-pi-season="${row.season}" ${expandedSeasons.has(String(row.season))?'open':''}><summary><span><strong>${row.season}</strong><small>${row.games_played} GP</small></span><span><small>Fantasy points</small><b>${num(row.fantasy_points,1)}</b></span><span><small>Fantasy PPG</small><b>${num(row.fantasy_ppg,1)}</b></span><span><small>Position rank</small><b>${row.position_rank?`#${row.position_rank}`:'—'}</b></span></summary><div class="pi-stat-grid">${positionStats(row)}</div><p class="pi-foot">${esc(row.scoring_basis)} · GP: ${esc(row.games_played_basis||'basis unavailable')} · ${esc(row.source)} / ${esc(row.source_version)}${row.rank_basis?` · ${esc(row.rank_basis)}`:''}</p></details>`).join('')}</div>`;
  }
