...
(function(){
  "use strict";
  const VERSION="20260925-market-current-beta-corrective1";
  ...
  const market={
    ...
    playerForecastSummary:null,
    playerIntrinsicSummary:null,
    ...
    finderFilters:{position:"",team:"",status:"",deal:"",assets:"",sort:"authority"},
    detailKey:"",
    pathKey:"",
  ...
  function finderPaths(payload){
    const f=market.finderFilters,intent=selectedIntent(),mode=finderMode(),selectedValue=String(intent.value||"");
    let paths=[...candidatePaths(payload)];
    const row=path=>representative(path)||{};
    if(mode!=="improve"&&!selectedValue)return[];
    if(mode==="position")paths=paths.filter(path=>String(row(path).target_position||"")===selectedValue);
    if(mode==="owner")paths=paths.filter(path=>String(path.counterparty_team_id||"")===selectedValue);
    if(mode==="target")paths=paths.filter(path=>(row(path).receive||[]).some(item=>String(item.asset_ref||"")===selectedValue));
    if(mode==="shop")paths=paths.filter(path=>(row(path).send||[]).some(item=>String(item.asset_ref||"")===selectedValue));
  ...
  function focusLifecycle(){
    return window.fsfflMarketFocus?.status?.()||{
      busy:false,dirty:false,needs_submit:false,error:"",configured:{...selectedIntent(),posture:selectedPosture()},
      submitted:null,completed:null,missing_required_value:false
    };
  }
  function focusCriteriaCopy(value){
    if(!value)return"";
    const mode=value.intent==="target"?"Target player":value.intent==="shop"?"Shop player":value.intent==="position"?"Target position":value.intent==="owner"?"Explore owner/team":"Improve team";
    const posture=POSTURES.find(row=>row[0]===value.posture)?.[1]||words(value.posture||"Calculated");
    return mode+(value.value?" · "+value.value:"")+" · "+posture;
  }
  function finderResultMarkup(payload,paths,lifecycle){
    if(lifecycle.busy){
      return loading("Finding opportunities","The submitted Search is running. No percentage progress is invented; prior results are hidden until this request completes.");
    }
    if(lifecycle.error){
      return unavailable("Search did not complete",lifecycle.error);
    }
    if(lifecycle.needs_submit){
      const copy=lifecycle.missing_required_value
        ?"Choose the required player, position, or owner, then submit the search."
        :"Your criteria changed. Prior results are stale and are hidden until you submit the new search.";
      return unavailable("Ready to find opportunities",copy);
    }
    if(paths.length){
      return paths.slice(0,80).map(path=>{
        const r=representative(path),a=pathAuthority(path);
        return"<article><button type='button' data-finder-open='"+esc(path.path_id)+"'><div><small>"+esc(r.target_position||"Trade path")+" · "+esc(r.counterparty_name||"Owner unavailable")+"</small><strong>"+esc(labels(r.receive).join(" + ")||"Target unavailable")+"</strong><span>"+esc(words(path.bilateral_plausibility))+" · "+esc(words(path.economic_screen))+"</span></div><span class='market-ns-status "+a.key+"'>"+esc(a.label)+"</span><b>"+esc((path.alternate_packages||[]).length+1)+" package variant"+(((path.alternate_packages||[]).length+1)===1?"":"s")+"</b></button></article>";
      }).join("");
    }
    const outcome=payload?.trade_discovery?.focus_outcome||lifecycle.completed?.outcome||null;
    if(outcome?.status==="zero"){
      return unavailable("No candidate path survived this submitted search",outcome.message||"The requested neighborhood was explored but did not produce a surviving candidate path.");
    }
    return unavailable("No paths match this view","The submitted Search may have results outside the current advanced filters.");
  }
  function renderTradeFinder(body,payload){
    ...
    const lifecycle=focusLifecycle(),mode=finderMode(),f=market.finderFilters,paths=finderPaths(payload),calc=words(currentCalculatedState(payload)),diag=marketDiscovery(payload)?.diagnostics||{},screened=Number(diag.preliminary_decision_runs||0),families=Number(diag.path_families_created||candidatePaths(payload).length||0),searchOnly=payload?.delivery?.completeness==="search_only",screenCopy=searchOnly?"Preliminary screening is still running. Search-only matches remain explorable, not recommendations.":screened+" of "+families+" path families received the bounded preliminary Decision screen; unscreened paths remain explicitly marked.";
    const configured=focusCriteriaCopy(lifecycle.configured),completed=focusCriteriaCopy(lifecycle.completed),submitDisabled=lifecycle.busy||lifecycle.missing_required_value;
    const lifecycleCopy=lifecycle.busy
      ?"Running submitted criteria: "+focusCriteriaCopy(lifecycle.submitted)
      :lifecycle.needs_submit
        ?"Configured, not submitted: "+configured
        :completed
          ?"Completed criteria: "+completed
          :"Current automatic workspace; change criteria and submit to run focused discovery.";
    const primary="<div class='market-ns-finder-primary'><label><span>Strategic lens</span><select data-finder-posture>"+POSTURES.map(([key,label])=>"<option value='"+key+"'"+(selectedPosture()===key?" selected":"")+">"+label+"</option>").join("")+"</select></label>"+intentValueControl(mode)+"<div class='market-ns-submit-row'><button type='button' class='primary-button' data-finder-submit "+(submitDisabled?"disabled":"")+">"+(lifecycle.busy?"Finding opportunities…":"Find opportunities")+"</button><span aria-live='polite'>"+esc(lifecycleCopy)+"</span></div></div>";
    ...
    const visibleCount=(lifecycle.busy||lifecycle.needs_submit)?0:paths.length;
    body.innerHTML="<section class='market-ns-section-head'><div><p class='eyebrow'>Trade Finder</p><h2>What are you trying to do?</h2><p>Configure first, then submit. Calculated competitive state remains "+esc(calc)+"; the strategic lens governs Search admission only where current evidence supports it.</p><p class='market-ns-screen-coverage'>"+esc(screenCopy)+"</p></div><small>"+visibleCount+" candidate path"+(visibleCount===1?"":"s")+"</small></section><div class='market-ns-finder'><div class='market-ns-mode-grid'>"+[["improve","Improve my team"],["target","Target a player"],["shop","Shop a player"],["position","Target a position"],["owner","Explore an owner / team"]].map(([key,label])=>"<button type='button' data-finder-mode='"+key+"' class='"+(mode===key?"active":"")+"'>"+label+"</button>").join("")+"</div>"+primary+advanced+"</div><div class='market-ns-result-list'>"+finderResultMarkup(payload,paths,lifecycle)+"</div>";
  ...
  function wireFinder(body,payload,mode){
    ...
    body.querySelector("[data-finder-intent-value]")?.addEventListener("change",event=>{if(mode!=="improve"){window.fsfflMarketIntent?.set?.(mode,event.target.value||"");renderBody(payload)}});
    body.querySelector("[data-finder-posture]")?.addEventListener("change",event=>{window.fsfflOpportunityPosture?.setPosture?.(event.target.value);renderBody(payload)});
    body.querySelector("[data-finder-submit]")?.addEventListener("click",()=>window.fsfflMarketFocus?.submit?.());
    ...
    body.querySelector("[data-finder-reset]")?.addEventListener("click",()=>{market.finderFilters={position:"",team:"",status:"",deal:"",assets:"",sort:"authority"};renderBody(payload)});
  ...
  async function ensurePlayerUniverse({refreshOptional=false}={}){
    if((market.playerLoaded&&!refreshOptional)||market.playerLoading)return;
    market.playerLoading=true;if(!refreshOptional)market.playerError=null;renderBody(opp()?.payload||null);
    try{
      const results=await Promise.all([
        api("/api/league/value-lenses?universe=all"),
        api("/api/league/team-views").catch(()=>({team_views:[]}))
      ]);
      const lenses=results[0]||{},league=results[1]||{};
      market.playerReadiness=lenses.surface_readiness||null;
      market.playerForecastSummary=lenses.all_player_forecast||null;
      market.playerIntrinsicSummary={
        ...(lenses.fsffl_intrinsic||{}),
        execution:lenses.intrinsic_execution||null
      };
      ...
      market.playerRows=(lenses.players||[]).map(row=>{
        const a=analytics.get(String(row.player_id)),serverProjection=finite(row.season_fantasy_points_projection)?row.season_fantasy_points_projection:null,projection=serverProjection??(a?projectionFromPlayer(a):null),serverPpg=finite(row.season_ppg_17)?row.season_ppg_17:null;
        return{
          ...
          projected_starter:a?.projected_starter===true,role:a?(a.projected_starter?"starter":"reserve"):(row.roster_status==="available"?"available":"unavailable"),
          projection,ppg:serverPpg??(finite(projection)?projection/17:null),forecast_status:row.season_forecast_status||"unavailable",forecast_reason:row.season_forecast_reason||null,
          market_status:row.broad_market_status||"unavailable",market_reason:row.broad_market_reason||null,
          intrinsic_status:row.intrinsic_status||"unavailable",intrinsic_reason:row.intrinsic_reason||null,
          market_index:row.broad_market_value_index,intrinsic_index:row.intrinsic_value_index,
          market_percentile:row.broad_market_percentile,intrinsic_percentile:row.intrinsic_percentile,value_gap:(finite(row.broad_market_value_index)&&finite(row.intrinsic_value_index)?row.intrinsic_value_index-row.broad_market_value_index:null),overall_rank:null,position_rank:null
        };
      });
      deriveRanks(market.playerRows);market.playerLoaded=true;
      if(lenses?.fsffl_intrinsic?.status==="building"||lenses?.surface_readiness?.status==="building_optional"){
        clearTimeout(market.playerRefreshTimer);
        const retry=Math.max(750,Math.min(5000,Number(lenses.fsffl_intrinsic?.retry_after_ms||lenses.surface_readiness?.retry_after_ms||1500)));
        market.playerRefreshTimer=setTimeout(()=>{market.playerLoaded=false;void ensurePlayerUniverse({refreshOptional:true})},retry);
      }
  ...
  function boardSortHeader(key,label,{secondary=false}={}){
    const active=market.boardSort.key===key,dir=active?market.boardSort.direction:"none",arrow=active?(dir==="asc"?" ↑":" ↓"):"";
    return"<th"+(secondary?" class='market-ns-secondary-col'":"")+" aria-sort='"+(active?(dir==="asc"?"ascending":"descending"):"none")+"'><button type='button' class='market-ns-sort-head"+(active?" active":"")+"' data-board-sort-key='"+esc(key)+"'>"+esc(label)+arrow+"</button></th>";
  }
  function playerRowMarkup(row){
    return"<tr><td class='market-ns-player-sticky'><button type='button' class='pi-player-link market-ns-player-link' data-player-intelligence-id='"+esc(row.player_id)+"' "+playerActionContext(row)+"><strong>"+esc(row.full_name)+"</strong><small>"+esc(row.position)+" · "+esc(row.nfl_team||"NFL team unavailable")+"</small></button></td><td>"+age(row.age)+"</td><td>"+esc(row.owner_team_name||(row.roster_status==="available"?"Available":"Unavailable"))+"</td><td title='"+esc(row.market_reason||"")+"'>"+integer(row.market_index)+"</td><td title='"+esc(row.intrinsic_reason||"")+"'>"+integer(row.intrinsic_index)+"</td><td class='market-ns-secondary-col'>"+(finite(row.market_index)&&finite(row.intrinsic_index)?integer(row.intrinsic_index-row.market_index):"Unavailable")+"</td><td class='market-ns-secondary-col'>"+num(row.ppg,1)+"</td><td class='market-ns-secondary-col'>"+num(row.projection,1)+"</td><td class='market-ns-secondary-col'>"+(row.overall_rank?"#"+row.overall_rank:"Unavailable")+"</td><td class='market-ns-secondary-col'>"+(row.position_rank?"#"+row.position_rank+" "+esc(row.position):"Unavailable")+"</td></tr>";
  }
  function renderPlayerBoard(body){
    ...
    const f=market.boardFilters,rows=boardRows(),positions=market.playerRows.map(x=>x.position),owners=market.playerRows.filter(x=>x.owner_team_id).map(x=>x.owner_team_name),nfl=market.playerRows.map(x=>x.nfl_team),forecast=market.playerForecastSummary||{},intrinsic=market.playerIntrinsicSummary||{};
    const intrinsicBuilding=intrinsic.status==="building"||market.playerReadiness?.status==="building_optional",intrinsicReadyCount=market.playerRows.filter(row=>row.intrinsic_status==="ready").length,forecastCopy=Number(forecast.covered_players||0)+" / "+Number(forecast.requested_players||market.playerRows.length)+" players have governed full-season Forecast coverage.",intrinsicCopy=intrinsicBuilding?"FSFFL Intrinsic is preparing for the current State/Forecast coordinate; Broad Market remains live.":intrinsic.status==="unavailable"?"FSFFL Intrinsic unavailable: "+String(intrinsic.reason||intrinsic.execution?.error||"current governed prerequisite is unavailable"):"FSFFL Intrinsic covers "+intrinsicReadyCount+" / "+market.playerRows.length+" players.";
    body.innerHTML="<section class='market-ns-section-head'><div><p class='eyebrow'>Player Board</p><h2>See the player market.</h2><p>Broad Market and FSFFL Intrinsic stay separate. "+esc(intrinsicCopy)+" "+esc(forecastCopy)+"</p></div><div class='market-ns-section-actions'><small>"+rows.length+" players</small><button type='button' class='text-button' data-market-disagreement>Explore Value disagreement</button></div></section><details class='market-ns-advanced market-ns-board-advanced'><summary>Filters & ordering</summary><div class='market-ns-board-controls'><input type='search' data-board-filter='query' value='"+esc(f.query)+"' placeholder='Search player, owner, NFL team or keyword'><select data-board-filter='position'><option value=''>All positions</option>"+optionList(positions,f.position)+"</select><select data-board-filter='age'><option value=''>All ages</option><option value='u24'"+(f.age==="u24"?" selected":"")+">Under 24</option><option value='24-26'"+(f.age==="24-26"?" selected":"")+">24-26</option><option value='27-29'"+(f.age==="27-29"?" selected":"")+">27-29</option><option value='30p'"+(f.age==="30p"?" selected":"")+">30+</option></select><select data-board-filter='owner'><option value=''>All fantasy teams</option>"+unique(market.playerRows.filter(x=>x.owner_team_id).map(x=>[x.owner_team_id,x.owner_team_name])).map(()=> "").join("")+"</select><select data-board-filter='nfl'><option value=''>All NFL teams</option>"+optionList(nfl,f.nfl)+"</select><select data-board-filter='roster'><option value=''>All roster statuses</option><option value='managed'"+(f.roster==="managed"?" selected":"")+">My roster</option><option value='rostered'"+(f.roster==="rostered"?" selected":"")+">Rostered</option><option value='available'"+(f.roster==="available"?" selected":"")+">Available</option></select><select data-board-filter='role'><option value=''>All projected roles</option><option value='starter'"+(f.role==="starter"?" selected":"")+">Projected starters</option><option value='reserve'"+(f.role==="reserve"?" selected":"")+">Projected reserves</option></select><select data-board-sort><option value='market_index'>Broad Market</option><option value='intrinsic_index'>FSFFL Intrinsic</option><option value='age'>Age</option><option value='ppg'>PPG</option><option value='projection'>17-game projection</option><option value='overall_rank'>Overall rank</option><option value='position_rank'>Position rank</option><option value='full_name'>Player name</option></select></div></details><div class='market-ns-board-meta'><span>Broad Market and FSFFL Intrinsic are shown side by side and never blended.</span><span>Ranks are presentation ordering only · League Market Value: unavailable · Team Utility: not part of this board.</span></div><div class='market-ns-player-table'><table><thead><tr>"+boardSortHeader("full_name","Player")+boardSortHeader("age","Age",{secondary:true})+"<th class='market-ns-secondary-col'>Fantasy team</th>"+boardSortHeader("market_index","Broad Market")+boardSortHeader("intrinsic_index","FSFFL Intrinsic")+boardSortHeader("value_gap","Δ",{secondary:true})+boardSortHeader("ppg","PPG",{secondary:true})+boardSortHeader("projection","17-game projection",{secondary:true})+boardSortHeader("overall_rank","Overall rank",{secondary:true})+boardSortHeader("position_rank","Position rank",{secondary:true})+"</tr></thead><tbody>"+(rows.length?rows.map(playerRowMarkup).join(""):"<tr><td colspan='10'>No players match these filters.</td></tr>")+"</tbody></table></div>";
    ...
    body.querySelectorAll("[data-board-sort-key]").forEach(button=>button.addEventListener("click",()=>{const key=button.dataset.boardSortKey,ascendingDefault=["age","full_name","overall_rank","position_rank"].includes(key),direction=market.boardSort.key===key?(market.boardSort.direction==="asc"?"desc":"asc"):(ascendingDefault?"asc":"desc");market.boardSort={key,direction};renderPlayerBoard(body)}));
  }

  function freeFitReason(row){
    const matches=tradeRows().filter(item=>String(item.target_position||"")===String(row.position||"")&&finite(item.focal_position_strength_rank));
    if(matches.length){
      matches.sort((a,b)=>(a.focal_position_strength_index??Infinity)-(b.focal_position_strength_index??Infinity));
      return "Addresses "+row.position+" depth · current league position rank #"+matches[0].focal_position_strength_rank+".";
    }
    if(finite(row.projection))return "Forecast available; team-specific roster-fit ranking is not available for this player.";
    return "Roster-fit evidence unavailable until governed Forecast coverage is available.";
  }
  function freeRows(){
  ...
  function renderFreeAgents(body){
    ...
    const f=market.freeFilters,rows=freeRows(),nfl=market.playerRows.filter(x=>x.roster_status==="available").map(x=>x.nfl_team),forecast=market.playerForecastSummary||{},intrinsic=market.playerIntrinsicSummary||{},availableForecasted=market.playerRows.filter(x=>x.roster_status==="available"&&x.forecast_status==="ready").length,availableIntrinsic=market.playerRows.filter(x=>x.roster_status==="available"&&x.intrinsic_status==="ready").length,availableTotal=market.playerRows.filter(x=>x.roster_status==="available").length,intrinsicNote=intrinsic.status==="building"?"Intrinsic is preparing.":intrinsic.status==="unavailable"?"Intrinsic unavailable: "+String(intrinsic.reason||intrinsic.execution?.error||"governed prerequisite unavailable"):"Intrinsic coverage "+availableIntrinsic+" / "+availableTotal+".";
    body.innerHTML="<section class='market-ns-section-head'><div><p class='eyebrow'>Free Agents</p><h2>Available players</h2><p>No waiver priority is invented. Governed full-season Forecast coverage is "+availableForecasted+" / "+availableTotal+" available players; uncovered Forecast stays unavailable. "+esc(intrinsicNote)+"</p></div><small>"+rows.length+" available</small></section>"+waiverEvaluator()+"<div class='market-ns-free-controls'><input type='search' data-free-filter='query' value='"+esc(f.query)+"' placeholder='Search available player'><select data-free-filter='position'><option value=''>All positions</option>"+optionList(["QB","RB","WR","TE"],f.position)+"</select><select data-free-filter='nfl'><option value=''>All NFL teams</option>"+optionList(nfl,f.nfl)+"</select><select data-free-sort><option value='market_index'>Broad Market</option><option value='intrinsic_index'>FSFFL Intrinsic</option><option value='projection'>17-game projection</option><option value='ppg'>PPG</option><option value='age'>Age</option><option value='position_rank'>Position rank</option></select></div><div class='market-ns-free-list'>"+(rows.length?rows.map(row=>"<article><button type='button' class='market-ns-free-player pi-player-link' data-player-intelligence-id='"+esc(row.player_id)+"' "+playerActionContext(row)+"><span><strong>"+esc(row.full_name)+"</strong><small>"+esc(row.position)+" · "+esc(row.nfl_team||"NFL team unavailable")+" · "+esc(freeFitReason(row))+"</small></span><span><small>PPG</small><b>"+num(row.ppg,1)+"</b></span><span><small>17-game</small><b>"+num(row.projection,1)+"</b></span><span><small>Broad Market</small><b>"+integer(row.market_index)+"</b></span><span><small>Intrinsic</small><b>"+integer(row.intrinsic_index)+"</b></span></button><button type='button' class='secondary-button' data-free-evaluate='"+esc(row.player_id)+"'>Evaluate add/drop</button></article>").join(""):unavailable("No available players match","Change the filters to broaden the list."))+"</div>";
  ...
  function reset(){
    clearTimeout(market.playerRefreshTimer);market.playerRefreshTimer=null;market.playerRows=[];market.playerLoaded=false;market.playerLoading=false;market.playerError=null;market.playerReadiness=null;market.playerForecastSummary=null;market.playerIntrinsicSummary=null;market.detailKey="";market.pathKey="";market.waiverPlayer=null;market.waiverResult=null;market.waiverSeq+=1;
  }
  ...
  window.addEventListener("fsffl:market-focus-state-changed",()=>{if(onMarket()&&market.tab==="trade_finder")renderBody(opp()?.payload||null)});
})();