...
(function(){
  ...
  function card(row,index,featured=false){const receive=assets(row.receive),send=assets(row.send),tone=decisionTone(row),owner=ownerSignal(row),target=row.target_position||'Roster fit';return `<article class="ns-market-card ${tone}${featured?' featured':''}" data-ns-market-card="${index}">
  ...
  function enhanceResult(){const result=document.querySelector('.opp-trade-result');if(!result||result.classList.contains('ns-market-result'))return;result.classList.add('ns-market-result');const heading=result.querySelector('.opp-result-head h3');if(heading)heading.insertAdjacentHTML('beforebegin','<span class="ns-market-result-icon" aria-hidden="true">↗</span>');const metrics=result.querySelector('.opp-result-metrics');if(metrics)metrics.setAttribute('aria-label','Changed-roster outcome summary')}
  function simplifyShell(){const panel=document.querySelector('#generic-screen .panel');if(!panel||state()?.payload?.status!=='ready')return;if(panel.classList.contains('ns-market-shell'))return;panel.classList.add('ns-market-shell');const title=panel.querySelector('.panel-header h2');if(title)title.textContent='Your market';const lead=panel.querySelector(':scope > .lead');if(lead)lead.textContent='Trade ideas shaped by your roster, the other franchise, and current FSFFL evidence.';const summary=panel.querySelector('.opp-summary');if(summary)summary.classList.add('ns-market-summary');const note=document.querySelector('#opportunity-body > .opp-note');if(note)note.classList.add('ns-market-supporting')}
  function enhance(){queued=false;if(typeof state==='function'&&state()){simplifyShell();renderDeck();enhanceResult()}}
