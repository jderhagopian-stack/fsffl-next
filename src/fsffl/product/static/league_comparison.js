...
function laPositionCell(view,position){
  ...
  const mode=fsfflLeagueStructureState.positionLensMode||'rank',display=mode==='strength'?laNum(row.strength_index,0):'#'+row.league_rank;
  const label=mode==='strength'?'strength index '+laNum(row.strength_index,0):'rank #'+row.league_rank+' of '+count;
  return '<button type="button" class="league-edge-cell '+band+'" data-room-team="'+laEsc(view.team_id)+'" data-room-position="'+laEsc(position)+'" aria-label="'+laEsc(view.display_name)+' '+laEsc(position)+' '+laEsc(label)+'; open team-position detail" title="'+laEsc(position)+' rank #'+row.league_rank+' of '+count+'; strength index '+laNum(row.strength_index,0)+'"><b>'+display+'</b></button>';
}
function laPositionMatrix(){
  const positions=['QB','RB','WR','TE'],mode=fsfflLeagueStructureState.positionLensMode||'rank',views=[...fsfflLeagueStructureState.views].sort((a,b)=>{if(a.team_id===fsfflLeagueStructureState.managedTeamId)return-1;if(b.team_id===fsfflLeagueStructureState.managedTeamId)return 1;return a.display_name.localeCompare(b.display_name)});
  ...
  const helper=mode==='strength'?'100 = league-average optimized starter production. Existing league-relative evidence bands are preserved.':'League-relative position rank, #1 through #'+views.length+'. Team order does not change between lenses.';
  const controls='<div class="league-position-lens-wrap"><div class="league-position-lens" role="group" aria-label="Position map lens"><button type="button" data-position-lens="rank" class="'+(mode==='rank'?'active':'')+'">Rank</button><button type="button" data-position-lens="strength" class="'+(mode==='strength'?'active':'')+'">Strength Index</button></div><small>'+helper+'</small></div>';
  return '<section class="league-section league-position-section"><div class="league-section-heading league-map-heading"><div><p class="eyebrow">League Map</p><h3>See positional control at a glance</h3></div>'+controls+'</div><div class="league-edge-matrix league-edge-map">'+header+rows+'</div><p class="atlas-foot league-map-foot">Tap any position to see the players and evidence behind its rank.</p></section>';
}
...
function installLeagueComparisonStyles(){
  if(!document.querySelector('link[data-fsffl-league-atlas-css]')){const link=document.createElement('link');link.rel='stylesheet';link.dataset.fsfflLeagueAtlasCss='true';link.href='/static/league_atlas.css?v=20260923-league-atlas-finaliphone7';document.head.appendChild(link)}
