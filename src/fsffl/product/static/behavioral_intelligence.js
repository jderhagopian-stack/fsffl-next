...
function fsfflBehaviorSpan(profile){const seasons=profile?.seasons_observed||[];if(seasons.length===1)return String(seasons[0]);if(seasons.length>1)return`${seasons[0]}–${seasons[seasons.length-1]}`;return'—'}
function fsfflBehaviorName(profile){return profile?.current_team_name||`Owner ${String(profile?.owner_id||'').slice(0,8)}`}
