...
(function(){
  'use strict';
  const VERSION='20260914-fundamental-intrinsic-v5';
  ...
  function mainDriver(estimate,presentation){
    ...
    if(terminal&&finite(terminal.continuation_value)){
      const pedigree=finite(terminal.pedigree_residual_value)?` Residual draft pedigree contributes ${terminal.pedigree_residual_value>=0?'+':''}${terminal.pedigree_residual_value.toFixed(1)} shared football-value units after removing the portion explained by Forecast.`:' Exact draft-pick pedigree is unavailable, so no residual adjustment is fabricated.';
  ...
  function coordinateCards(payload){
    ...
    return `<div class="value-lens-coordinates" aria-label="FSFFL value coordinates">
      ...
      <article class="primary"><span>FSFFL Intrinsic Value</span><strong>${hasIntrinsic?'What FSFFL believes the dynasty asset is fundamentally worth':'Currently unavailable'}</strong><p>Independent long-term football value on one shared cross-position magnitude: governed Y1–Y3 Forecast distributions, state-conditioned post-Y3 continuation, and only validated residual fundamental evidence.</p><small>${hasIntrinsic?`Model ${esc(payload.model_version||'intrinsic-fundamental-career-value-v5')}`:'No substitute value is fabricated'}</small></article>
  ...
  function rowHtml(row){
    ...
    return `<details class="value-lens-player" data-signal="${cmp.kind}">
      ...
      <div class="value-lens-player-detail">
        ...
        ${available?`<details class="value-lens-provenance"><summary>Evidence & provenance</summary><p><b>Displayed Intrinsic:</b> ${valueNumber(estimate.display_value)} / 10,000<br><b>Shared fundamental coordinate:</b> ${finite(estimate.fundamental_value)?estimate.fundamental_value.toFixed(1):'—'} discounted football units<br><b>Raw expected career production:</b> ${finite(estimate.raw_fundamental_career_value)?estimate.raw_fundamental_career_value.toFixed(1):'—'} discounted football units<br><b>Residual fundamental contribution:</b> ${finite(estimate.residual_fundamental_value)?estimate.residual_fundamental_value.toFixed(1):'—'}<br><b>Uncertainty:</b> ${finite(estimate.fundamental_stddev)?estimate.fundamental_stddev.toFixed(1):'—'} shared football units<br><b>Terminal model:</b> ${esc(estimate.terminal?.model_version||'—')}<br><b>Calibration:</b> ${esc(estimate.calibration_version||'—')}<br><b>Intrinsic model:</b> ${esc(estimate.model_version||'—')}<br><b>Display scale:</b> ${esc(estimate.display_scale_version||'—')}<br><b>Forecast policy:</b> ${esc(estimate.forecast_policy_version||'—')}<br><b>Base Forecast:</b> ${esc(estimate.base_forecast_model_version||'—')}<br><b>As of:</b> ${esc(estimate.evaluation_as_of||'—')}</p></details>`:''}
      </div>
    </details>`;
  }
  ...
  function render(payload){
    ...
    host.innerHTML=`<div class="value-lens-shell">
      ...
      <details class="value-lens-methods"><summary>What exactly is FSFFL Intrinsic Value?</summary><p>Fundamental Intrinsic is a market-independent long-term dynasty asset coordinate on one shared cross-position football magnitude. It discounts governed Year 1–3 Forecast distributions, adds empirically validated state-conditioned post-Year-3 continuation, then adds only residual draft-pedigree information after removing the portion explained by the Forecast vector. The final coordinate is not normalized separately by position. Replacement surplus, Broad Market, League Market, Team Utility and owner behavior are not Intrinsic inputs.</p><p>The 0–10,000 presentation is a separate monotonic mapping from fixed football-only shared-magnitude anchors. Its reference universe excludes the historical fringe half of repaired predictions and reserves the apex band for genuinely extreme football-career magnitude. It is never fit to market prices.</p></details>
    </div>`;
