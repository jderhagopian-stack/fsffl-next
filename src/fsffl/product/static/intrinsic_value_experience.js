...
(function(){
  ...
  function coordinateCards(payload){
    ...
    return `<div class="value-lens-coordinates" aria-label="FSFFL value coordinates">
      ...
      <article class="primary"><span>FSFFL Intrinsic · Shapley</span><strong>${hasIntrinsic?'What FSFFL football economics imply':'Currently unavailable'}</strong><p>Governed three-year marginal fantasy-point contribution in the FSFFL deployment game. Market price is not an input.</p><small>${hasIntrinsic?`Contract ${esc(payload.contract_version||'intrinsic-shapley-contract')}`:'No substitute value is fabricated'}</small></article>
  ...
  function rowHtml(row,payload){
    ...
    return `<details class="value-lens-player" data-signal="${cmp.kind}">
      <summary>
        ...
        <span><small>FSFFL Intrinsic</small><strong>${estimate?`${raw(intrinsicValue(estimate))} marginal pts`:'Unavailable'}</strong>${finite(row.intrinsicRank)?`<small>${pct(row.intrinsicRank)} display rank</small>`:''}</span>
      ...
      <div class="value-lens-player-detail">
        ...
        ${estimate?`<details class="value-lens-provenance"><summary>Evidence & provenance</summary><p><b>Intrinsic contract:</b> ${esc(payload.contract_version||'—')}<br><b>Intrinsic model:</b> ${esc(payload.intrinsic_model_version||'—')}<br><b>Forecast model:</b> ${esc(payload.forecast_model_version||'—')}<br><b>Quantity:</b> ${esc(payload.quantity_semantics||'—')}<br><b>Discount:</b> ${finite(payload.discount)?payload.discount.toFixed(2):'—'}<br><b>Simulation permutations:</b> ${finite(payload.permutations)?payload.permutations.toLocaleString():'—'}</p></details>`:''}
      </div>
    </details>`;
  }
  ...
  function render(payload){
    ...
    host.innerHTML=`<div class="value-lens-shell">
      ...
      <section class="value-lens-read"><div class="value-lens-section-head"><div><p class="eyebrow">Where the lenses disagree</p><h3>Start with the biggest differences.</h3></div><span>${comparable.length} roster players comparable</span></div>
        <p class="value-lens-note">The comparison uses percentile rank only as a presentation aid because Broad Market and Shapley Intrinsic use different units. FSFFL does <strong>not</strong> subtract the raw numbers or turn disagreement into a buy/sell command. League-specific pricing and Team Utility still matter.</p>
