...
(function(){
  'use strict';
  const VERSION='20260914-fundamental-intrinsic-v6-scale2';
  ...
  function comparison(intrinsicRank,marketRank,available){
    ...
    if(delta>=.18)return{kind:'intrinsic',label:'FSFFL materially higher',copy:'FSFFL’s fundamental dynasty-value rank is materially higher than the broad dynasty market. Investigate the gap; it is not an automatic buy signal.'};
    if(delta>=.07)return{kind:'intrinsic',label:'FSFFL moderately higher',copy:'FSFFL’s fundamental dynasty-value rank is moderately higher than the broad dynasty market. Investigate the gap before acting.'};
    ...
    if(delta<=-.07)return{kind:'market',label:'Broad market moderately higher',copy:'The broad dynasty market ranks this player materially higher than FSFFL fundamentals do. Investigate the gap before acting.'};
  ...
  function mainDriver(estimate,presentation){
    ...
    if(terminal&&finite(terminal.continuation_value)){
      const pedigree=finite(terminal.pedigree_residual_value)?` Residual draft pedigree contributes ${terminal.pedigree_residual_value>=0?'+':''}${terminal.pedigree_residual_value.toFixed(1)} pre-structural football units after removing the portion explained by Forecast.`:' Exact draft-pick pedigree is unavailable, so no residual adjustment is fabricated.';
      return `Years 1–3 contribute ${Math.round(estimate.raw_discounted_y1_y3||0)} discounted football units; governed state-conditioned post-Year-3 continuation contributes ${Math.round(terminal.continuation_value)}.${pedigree}`;
    }
  ...
  function coordinateCards(payload){
    ...
    return `<div class="value-lens-coordinates" aria-label="FSFFL value coordinates">
      <article><span>Broad Market Value</span><strong>What the broader dynasty market appears to charge</strong><p>Primary magnitude uses the governed 0–10,000 market-cardinal coordinate; market percentile remains secondary context.</p><small>External market evidence · separate from FSFFL fundamentals</small></article>
      <article class="primary"><span>FSFFL Intrinsic Value</span><strong>${hasIntrinsic?'What FSFFL independently estimates the player is worth long term':'Currently unavailable'}</strong><p>Expected career production, career trajectory, longevity/risk and residual fundamental evidence are translated through league-format starter pressure and effective positional supply.</p><small>${hasIntrinsic?`Model ${esc(payload.model_version||'intrinsic-fundamental-economic-value-v6')}`:'No substitute value is fabricated'}</small></article>
      <article class="unavailable"><span>League Market Value</span><strong>Not production-ready</strong><p>Future coordinate for what this specific league appears to charge for the asset.</p><small>Unavailable by design</small></article>
  ...
  function rowHtml(row){
    ...
    return `<details class="value-lens-player" data-signal="${cmp.kind}">
      ...
      <div class="value-lens-player-detail">
        ...
        ${available?`<details class="value-lens-provenance"><summary>Evidence & provenance</summary><p><b>Displayed Intrinsic:</b> ${valueNumber(estimate.display_value)} / 10,000<br><b>Economic Intrinsic coordinate:</b> ${finite(estimate.fundamental_value)?estimate.fundamental_value.toFixed(1):'—'} league-structural fundamental units<br><b>Pre-structural career value:</b> ${finite(estimate.pre_structural_fundamental_value)?estimate.pre_structural_fundamental_value.toFixed(1):'—'} discounted football units<br><b>Raw expected career production:</b> ${finite(estimate.raw_fundamental_career_value)?estimate.raw_fundamental_career_value.toFixed(1):'—'} discounted football units<br><b>Structural position factor:</b> ${finite(estimate.structural_factor)?estimate.structural_factor.toFixed(3):'—'}<br><b>Residual fundamental contribution:</b> ${finite(estimate.residual_fundamental_value)?estimate.residual_fundamental_value.toFixed(1):'—'}<br><b>Uncertainty:</b> ${finite(estimate.fundamental_stddev)?estimate.fundamental_stddev.toFixed(1):'—'} economic units<br><b>Terminal model:</b> ${esc(estimate.terminal?.model_version||'—')}<br><b>Calibration:</b> ${esc(estimate.calibration_version||'—')}<br><b>Intrinsic model:</b> ${esc(estimate.model_version||'—')}<br><b>Display scale:</b> ${esc(estimate.display_scale_version||'—')}<br><b>Forecast policy:</b> ${esc(estimate.forecast_policy_version||'—')}<br><b>Base Forecast:</b> ${esc(estimate.base_forecast_model_version||'—')}<br><b>As of:</b> ${esc(estimate.evaluation_as_of||'—')}</p></details>`:''}
      </div>
    </details>`;
  }
  ...
  function render(payload){
    ...
    host.innerHTML=`<div class="value-lens-shell">
      <header class="value-lens-hero"><div><p class="eyebrow">Value Lens</p><h3>Market price and fundamental dynasty worth are different questions.</h3><p><strong>Broad Market Value</strong> is what the broader dynasty market appears to charge. <strong>FSFFL Intrinsic Value</strong> is FSFFL’s independent estimate of what the player is actually worth long term from football fundamentals and league-format economics. Both use a familiar 0–10,000 presentation language, but neither is derived from the other.</p></div><div class="value-lens-count"><strong>${disagree}</strong><span>roster player${disagree===1?'':'s'} worth a closer look</span><small>Based on governed rank disagreement</small></div></header>
      ...
      <details class="value-lens-methods"><summary>What exactly is FSFFL Intrinsic Value?</summary><p>Fundamental Intrinsic is a market-independent estimate of long-term dynasty asset worth. It discounts governed Year 1–3 Forecast distributions, adds empirically validated state-conditioned post-Year-3 continuation, adds only residual draft-pedigree information after removing the portion already explained by Forecast, then converts that career-football magnitude through league-format starter pressure and effective positional production supply. The conversion is derived from lineup rules and governed Forecast distributions, not an arbitrary position bonus.</p><p>Replacement surplus, Broad Market, League Market, Team Utility, owner behavior and observed trade prices are not Intrinsic inputs. The 0–10,000 presentation is a monotonic concave translation of the completed economic coordinate, not a roster percentile. A square-root transform below a robust football-only historical apex handles the heavy right tail; only the extreme apex tail compresses toward 10,000. The single apex anchor is the all-positive PIT economic-raw p99 and is never fit to market prices, named players or a fixed league roster count.</p></details>
    </div>`;
