...
function myTeamDiagnosticTopValuedPlayer(players){const ranked=(players||[]).map(player=>({player,value:myTeamValueNumber(player.player_id)})).filter(row=>typeof row.value==='number'&&Number.isFinite(row.value)).sort((a,b)=>b.value-a.value);return ranked[0]||null}
function myTeamDiagnosticAssetText(row){return row?`${row.player.full_name} · ${Math.round(row.value).toLocaleString()}`:'Unavailable'}
...
function renderFsfflMyTeamDiagnostics(){
  ...
  const roster=view.players||[],starters=roster.filter(player=>player.projected_starter),nonstarters=roster.filter(player=>!player.projected_starter),strengths=myTeamDiagnosticStrengths(view),weakest=strengths[0]||null,strongest=strengths[strengths.length-1]||null,resilience=view.utility?.roster_resilience;
  const starterAge=myTeamDiagnosticAverageAge(starters),rosterAge=myTeamDiagnosticAverageAge(roster);
  const cornerstone=myTeamDiagnosticTopValuedPlayer(roster),nonstarterLeverage=myTeamDiagnosticTopValuedPlayer(nonstarters);
