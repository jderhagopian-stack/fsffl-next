...
_PRODUCT_MODEL_VERSION = "next8-trade-opportunity-action-v3:shared-behavioral-fit"
...
def build_trade_opportunity_evaluation(
    ...
    *,
    ...
    The shared post-trade runtime owns changed-state Simulation, NEXT-5 disposition
    and directional Behavioral fit. NEXT-6 maps those outputs into opportunity
    lifecycle authority. Behavioral fit does not rewrite Value, disposition, or
    action authority. Until a separately calibrated acceptance model exists, even
    a complete focal SUPPORT result is MARKET_TEST_ONLY rather than ACTIONABLE.
    """
    ...
    return {
        **comparison,
        ...
        "opportunity_explanation": (
            "Complete changed-state trade evidence is attached. The shared post-trade "
            "result may include directional Behavioral fit, but acceptance remains "
            "uncalibrated. A supported offer may therefore be promoted only to "
            "market-test authority, not an acceptance prediction or automatic action."
        ),
        "authority": {
