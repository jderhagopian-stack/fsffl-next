...
_behavioral_coordinator = BehavioralRuntimeCoordinator(
    store_factory=((lambda: _behavioral_store) if _behavioral_store is not None else default_behavioral_store),
    max_workers=2,
)
_sleeper_probe_source = SleeperLiveSource()
_full_refresh_seconds = max(1, int(os.getenv("FSFFL_FULL_PROVIDER_REFRESH_SECONDS", "3600")))
