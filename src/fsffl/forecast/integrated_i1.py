...
THRESHOLDS = (
    frozenset({"usable", "starter", "premium", "elite"}),
    frozenset({"starter", "premium", "elite"}),
    frozenset({"premium", "elite"}),
    frozenset({"elite"}),
)
...
class I1RegularizationPolicy:
    """Governed regularization policy for the frozen I1 architecture.

    C is scikit-learn's inverse L2 regularization strength. The selected research
    policy is 0.25 everywhere today. Component overrides exist only so a future
    separately-authorized calibration can replace the blanket value with one or
    multiple evidence-backed factors without redesigning the model interface.
    """
...
def feature_dict(row: CanonicalFootballState) -> dict[str, float]:
    """Exact provider-neutral feature semantics frozen by PR #146 I1."""
    ...
    d: dict[str, float] = {
        ...
        f"h={row.horizon}": 1.0,
        "exp": min(15.0, max(0.0, float(row.experience_years))) / 10.0,
    }
    exp_band = "0_1" if row.experience_years <= 1 else "2_3" if row.experience_years <= 3 else "4_6" if row.experience_years <= 6 else "7_plus"
    d[f"e={exp_band}"] = 1.0

    p = max(0.0, float(row.current_fantasy_points))
    q = max(0.0, float(row.prior_fantasy_points or 0.0))
    d.update({
        "lp": math.log1p(p) / 6.0,
        "lprev": math.log1p(q) / 6.0,
        "dpts": max(-2.0, min(2.0, (p - q) / 100.0)),
        "prev_cov": 1.0 if row.prior_fantasy_points is not None else 0.0,
    })

    if row.usage_evidence_coverage:
        d.update({
            f"role={row.role_band or 'unknown'}": 1.0,
            "u_cov": 1.0,
            "lopg": math.log1p(max(0.0, float(row.opportunity_per_game or 0.0))) / 4.0,
            "lg": math.log1p(max(0.0, float(row.games or 0.0))) / 3.0,
        })
    else:
        d.update({"role=unknown": 1.0, "u_cov": 0.0, "lopg": 0.0, "lg": 0.0})

    if not row.roster_evidence_coverage:
        d.update({"r_cov": 0.0, "i_cov": 0.0, "part_cov": 0.0})
        return d

    d.update({
        "r_cov": 1.0,
        "i_cov": 1.0 if row.availability_evidence_coverage else 0.0,
        "part_cov": 1.0 if row.participation_evidence_coverage else 0.0,
    })
    ...
    for name in ("injury_limited_weeks", "non_ir_injury_limited_weeks", "inactive_injury_limited_weeks", "reserve_injury_limited_weeks"):
        d["l" + name] = math.log1p(max(0.0, float(getattr(row, name) or 0.0))) / 3.0
    d["nonir"] = float(row.non_ir_injury_flag or 0.0)
    d["inactiveinj"] = float(row.inactive_injury_flag or 0.0)
    d["inj_attached"] = d["nonir"] * float(row.last_status_attached or 0.0)
...
class I1ModelArtifact:
    ...
    def predict(self, row: CanonicalFootballState, baseline: Mapping[str, float]) -> tuple[dict[str, float], str]:
        rich = row.has_full_i1_evidence
        persistence = self.persistence_full if rich else self.persistence_reduced
        ordered = self.ordered_full if rich else self.ordered_reduced
        path = "full" if rich else "red"
        if persistence is None or len(ordered) != 4:
            return _normalize(baseline), "b0_fallback"
        x = feature_dict(row)
        q = persistence.probability(x)
        cuts: list[float] = []
        last = 1.0
        for model in ordered:
            v = min(last, max(0.0, min(1.0, model.probability(x))))
            cuts.append(v); last = v
        a, b, c, d = cuts
        conditional = {"depth": 1-a, "usable": a-b, "starter": b-c, "premium": c-d, "elite": d}
        probs = {"out": 1-q, **{s: q * conditional[s] for s in POSITIVE_STATES}}
        return _normalize(probs), path


def _normalize(probs: Mapping[str, float]) -> dict[str, float]:
    out = {s: max(0.0, float(probs.get(s, 0.0))) for s in STATES}
    total = sum(out.values())
    if total <= 0:
        raise ValueError("career-state probability vector has no mass")
